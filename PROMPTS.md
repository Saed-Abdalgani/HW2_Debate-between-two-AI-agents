...
## Prompt 7 — HW2 Full Implementation & Chaos Testing (Phase 9)

**Tool:** Gemini 3.1 Pro
**Date:** 2026-05-18
**Context:** The codebase already included Phases P0 through P8, establishing the scaffold, config, IPC, supervisor logic, agents, menu, and state machine. I was asked to complete Phase 9: Validation, Chaos Engineering, and the Final Lab Report.

**Prompt Summary:**
```
Execute Phase 9 according to the todo.md file. Go step-by-step, write the necessary code, and update the checklist as you complete each individual task. Make sure that no files are longer than 150 lines.
```

**Purpose:** Provide full test coverage over failure modes defined in the PRD (child process crashes, network provider outages, invalid LLM json, token budget exhaustion). Run security audits, and generate the final documentation.

**Output Summary:**
- Wrote `tests/integration/test_recovery_chaos.py` simulating a `kill -9` of the `Pro` agent during a debate and verifying the parent's automatic heartbeat and state machine recovery.
- Wrote `tests/integration/test_budget_abort.py` to test the token limits forcing the `Gatekeeper` to raise a `BudgetExceeded` exception and gracefully aborting the sequence.
- Wrote `tests/integration/test_network_faults.py` using `respx` to inject HTTP latencies, `429 Too Many Requests` limiters, and JSON-parsing failures to verify backoff and retry integrity.
- Created `scripts/scan_secrets.py` that reliably scans the current workspace logs (`runs/`), stdout stream, and `git log -p` to prevent secret leakages (excluding test dummy keys).
- Addressed several runtime bugs in `watchdog` initialization and `JsonPipeWriter` exceptions to stabilize the chaos tests without violating the 150-line count.
- Wrote `README.md` as the final lab report containing architectures, Mermaid diagrams, config variables, and FSM documentation.
- Updated `PROMPTS.md` and checked off all items in the `TODO_HW2.md` file.

**Key Design Decisions Made by AI:**
1. In `test_recovery_chaos.py`, simulating a `os.kill(pid, signal.SIGTERM)` on the child process inside a `pulse` hook directly tests the interaction between the IPC pipeline (raising `ChildDisconnectedError` to the parent) and the asynchronous `watchdog` monitor triggering recovery FSM transitions (`RECOVER`, `RESPAWN`, `HEARTBEAT_MISS`).
2. Secret scanning (`scripts/scan_secrets.py`) ensures compliance but gracefully excludes hardcoded dummy API keys generated by existing unit tests (e.g. `sk-testkey123...`).
3. For network fault testing, rather than spinning up multiple fake web servers, `respx` mocked HTTP payloads allows complete manipulation of `httpx.Response` objects to instantly emulate timeout boundaries (`httpx.ReadTimeout`).

**Lessons Learned:**
- Chaos testing agent frameworks requires precise mock synchronization; when killing child processes, it isn't enough to just assert they die—the parent event loop must handle `BrokenPipeError` instantly before the next async heartbeat or polling cycle starts.
- Adhering strictly to a maximum 150 lines of code per file forces clean abstraction (e.g., pulling logic into `tests/integration/test_network_faults.py` and `tests/integration/test_recovery_chaos.py` rather than creating a massive integration monolithic test).
